package org.apache.solr.handler.dataimport.cache;
...
public class DIHCacheSupport {
  ...
  public DIHCacheSupport(Context context, String cacheImplName) {
    ...
    final String where = context.getEntityAttribute("where");
    String cacheKey = context.getEntityAttribute(DIHCacheSupport.CACHE_PRIMARY_KEY);
    final String lookupKey = context.getEntityAttribute(DIHCacheSupport.CACHE_FOREIGN_KEY);
    ...
    if (where == null && cacheKey == null) {
      ...
      if (where != null) {
        final String[] splits = where.split("=");
        cacheKey = splits[0];
  ...
  private DIHCache instantiateCache(Context context) {
    ...
    final
      Class<DIHCache> cacheClass = DocBuilder.loadClass(cacheImplName, context
          .getSolrCore());
      final Constructor<DIHCache> constr = cacheClass.getConstructor();
  ...
  public void initNewParent(Context context) {
    ...
    for (final Map.Entry<String,DIHCache> entry : queryVsCache.entrySet()) {
  ...
  public void destroyAll() {
    if (queryVsCache != null) {
      for (final DIHCache cache : queryVsCache.values()) {
        cache.destroy();
      }
    }
    queryVsCache = null;
  ...
  public void populateCache(String query,
    ...
    final DIHCache cache = queryVsCache.get(query);
  ...
  protected Map<String,Object> getIdCacheData(Context context, String query,
      Iterator<Map<String,Object>> rowIterator) {
    final Object key = context.resolve(cacheForeignKey);
    if (key == null) {
  ...
  protected Map<String,Object> getSimpleCacheData(Context context,
    ...
    if (dataSourceRowCache == null) {
      ...
      final Iterator<Map<String,Object>> cacheIter = queryVsCacheIterator.get(query);
  ...
  protected Map<String,Object> getFromRowCacheTransformed() {
    ...
    final Map<String,Object> r = dataSourceRowCache.next();
    return r;
  }
