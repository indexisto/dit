...
public class SortedMapBackedCache implements DIHCache {
  ...
  public void add(Map<String,Object> rec) {
    ...
    if (pk instanceof Collection<?>) {
      final Collection<Object> c = (Collection<Object>) pk;
      if (c.size() != 1) {
  ...
  public Iterator<Map<String,Object>> iterator(Object key) {
    ...
    if(key instanceof Iterable<?>) {
      final List<Map<String,Object>> vals = new ArrayList<Map<String,Object>>();
      final Iterator<?> iter = ((Iterable<?>) key).iterator();
      while(iter.hasNext()) {
        final List<Map<String,Object>> val = theMap.get(iter.next());
        if(val!=null) {
          vals.addAll(val);
        }
      }
    ...
    final List<Map<String,Object>> val = theMap.get(key);
    if (val == null) {
      return null;
    }
    return val.iterator();
  ...
  public void open(Context context) {
    ...
    final String pkName = CachePropertyUtil.getAttributeValueAsString(context,
        DIHCacheSupport.CACHE_PRIMARY_KEY);
    if (pkName != null) {
      primaryKeyName = pkName;
    }
    isReadOnly = false;
    final String readOnlyStr = CachePropertyUtil.getAttributeValueAsString(context,
