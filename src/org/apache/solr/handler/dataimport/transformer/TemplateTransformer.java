...
public class TemplateTransformer extends Transformer {
  ...
  private final Map<String ,List<String>> templateVsVars = new HashMap<String, List<String>>();

  @Override
  @SuppressWarnings("unchecked")
  public Object transformRow(Map<String, Object> row, Context context) {

    final VariableResolverImpl resolver = (VariableResolverImpl) context
            .getVariableResolver();
    // Add current row to the copy of resolver map
//    for (Map.Entry<String, Object> entry : row.entrySet())

    for (final Map<String, String> map : context.getAllEntityFields()) {
      final String expr = map.get(TEMPLATE);
      if (expr == null)
        continue;

      final String column = map.get(DataImporter.COLUMN);

      // Verify if all variables can be resolved or not
      boolean resolvable = true;
      final List<String> variables = getVars(expr);
  ...
  private List<String> getVars(String expr) {
    List<String> result = templateVsVars.get(expr);
    if(result == null){
      result = TemplateString.getVariables(expr);
      templateVsVars.put(expr, result);
    }
    return result;
  }
