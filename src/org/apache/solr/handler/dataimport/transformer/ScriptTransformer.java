...
public class ScriptTransformer extends Transformer {
 ...
 private String functionName;
  ...
  private void initEngine(Context context) {
    final String scriptText = context.getScript();
    final String scriptLang = context.getScriptLanguage();
    if (scriptText == null) {
      throw new DataImportHandlerException(SEVERE,
          "<script> tag is not present under <dataConfig>");
    }
    final ScriptEngineManager scriptEngineMgr = new ScriptEngineManager();
    final ScriptEngine scriptEngine = scriptEngineMgr.getEngineByName(scriptLang);
