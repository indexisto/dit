...
public class JdbcDataSource extends
  ...
  public void init(Context context, Properties initProps) {
    final Object o = initProps.get(CONVERT_TYPE);
    if (o != null)
    ...
    for (final Map<String, String> map : context.getAllEntityFields()) {
      final String n = map.get(DataImporter.COLUMN);
      final String t = map.get(DataImporter.TYPE);
...
//    final VariableResolver resolver = context.getVariableResolver();
    ...
    final String s = initProps.getProperty("maxRows");
    if (s != null) {
    ...
    public Connection call() throws Exception {
        ...
        try {
          if(url != null){
            c = DriverManager.getConnection(url + "?characterEncoding=UTF-8&characterSetResults=UTF-8", initProps);
          } else if(jndiName != null){
            final InitialContext ctx =  new InitialContext();
            final Object jndival =  ctx.lookup(jndiName);
            if (jndival instanceof javax.sql.DataSource) {
              final javax.sql.DataSource dataSource = (javax.sql.DataSource) jndival;
              final String user = (String) initProps.get("user");
              final String pass = (String) initProps.get("password");
          ...
          final Driver d = (Driver) DocBuilder.loadClass(driver, context.getSolrCore()).newInstance();
        ...
        if (c != null) {
          ...
          final String transactionIsolation = initProps.getProperty("transactionIsolation");
          ...
          final String holdability = initProps.getProperty("holdability");
        ...
        //LOG.info("Connection closed: " + c.isClosed());
        return c;
      }
    };
  }
  ...
  public Iterator<Map<String, Object>> getData(String query) {
    final ResultSetIterator r = new ResultSetIterator(query);
  ...
  private Connection getConnection() throws Exception {
    ...
    if (currTime - connLastUsed > CONN_TIME_OUT) {
      synchronized (this) {
        final Connection tmpConn = factory.call();
        closeConnection();
